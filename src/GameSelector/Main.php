...
class Main extends PluginBase implements Listener
{
    ...
    public $mode = 0;
    public $name= "";
    public $prefix = TextFormat::DARK_GRAY . "[" . TextFormat::GOLD . "Game" . TextFormat::GREEN . "Selector" . TextFormat::DARK_GRAY . "]" . TextFormat::RESET;

    public function OnEnable()
    {
        $this->getServer()->getPluginManager()->registerEvents($this, $this);
        //$this->getServer()->getLogger()->info($this->prefix . TextFormat::GREEN . "GameSelector has been enabled");
    ...
    public function OnCommand(CommandSender $sender, Command $command, string $label, array $args)
    {
        switch ($args[0]) {
            case "add":
                if ((!empty($args[1])) && !($this->config->getNested("Selectors.$args[1]"))) {
                    $this->name = $args[1];
                    $this->config->save();
                    $sender->sendMessage($this->prefix . TextFormat::GREEN . "Choose Selector now!");
                    $this->mode = 1;
                } else {
                    $sender->sendMessage($this->prefix . TextFormat::YELLOW . "Usage: /gs add < ad >");
                }
                break;
            case "additem":
                if ($this->config->getNested("Selectors.$args[1]") && (!empty($args[1])) && (!empty($args[2])) && $args[2] >= 0) {
                    $pos=$this->config->getNested("Selectors.$args[1]");
                    $x = $pos["x"];
                    $y = $pos["y"];
                    $z = $pos["z"];
                    $chest = $sender->getLevel()->getTile(new Vector3($x, $y, $z));
                    $chest->getInventory()->addItem(Item::get($args[2], $args[3], $args[4]));
                    $chest->saveNBT();
                    $sender->sendMessage($this->prefix . TextFormat::GOLD . $args[2] . " ID'li Item " . $args[1] . "'ye Eklendi!"); // fix here
                }else{
                    $sender->sendMessage($this->prefix.c::YELLOW."Usage: /gs additem <ad> <ItemID or Item ID, damage, count>");
                }
                break;
            case "setname":
                if($this->config->getNested("Selectors.$args[1]") and (!empty($args[1])) and (!empty($args[2]))){
                    ...
                    $chest->saveNBT();
                    $sender->sendMessage($this->prefix.c::YELLOW.$args[1]."'nin adı ".$args[2]." olarak değiştirildi!");
                    $sender->sendMessage($this->prefix.c::RED."Yapılan Ayarın Düzgün Kaydedilmesi bir müddet beklenmesi gerekir.");
                }else{
                    $sender->sendMessage($this->prefix.c::YELLOW."Usage: /gs setname <SelectorAdı> <isim>");
                }
                break;
            case "removeitem":
            case "deleteitem":
            case "delitem":
                if($this->config->getNested("Selectors.$args[1]") and (!empty($args[1])) and (!empty($args[2])) and $args[2]>=0){
                    ...
                    $chest->saveNBT();
                    $sender->sendMessage($this->prefix.c::RED.$args[2]." ID'li Item ".$args[1]."'den Silindi!");
                }else{
                    $sender->sendMessage($this->prefix.c::YELLOW."Usage: /gs additem <ad> <ItemID or Item ID, damage, count>");
                }
                break;
            case "setcommand":
                if($this->config->getNested("Selectors.$args[1]") and (!empty($args[1])) and (!empty($args[2])) and $args[2]>=0){
                    ...
                    $sender->sendMessage($this->prefix.c::YELLOW."Usage: /gs setcommand <ad> <ItemID or Item ID, damage, count> <command(/'sız)>");
                }
                break;
        }
    }
